...
import com.flink.datasource.MyNoParalleSource;
...
public class SteamingDemoWithMyParitition {
    public static void main(String[] args) throws Exception{
        ...
        DataStream<Tuple1<Long>> tupleData = text.map(new MapFunction<Long, Tuple1
                        <Long>>() {
            @Override
        ...
        //分区之后的数据
        DataStream<Tuple1<Long>> partitionData= tupleData.partitionCustom(new
                MyPartition(), 0);
        DataStream<Long> result = partitionData.map(new MapFunction<Tuple1<Long>,
                Long>() {
            @Override
